...
document.querySelector("input").addEventListener("keyup", event => {
    ...
    document.querySelector("#btn-new-project").click(); // Things you want to do.
...
function check_changes(isValid) {
    if (isValid) {
        if ($(".invalid-feedback").length == 0) {
            $("#btn-new-project").prop("disabled", false);
        } else if ($(".invalid-feedback").length == $(".invalid-feedback:hidden").length) {
            $("#btn-new-project").prop("disabled", false);
        }
    } else {
        $("#btn-new-project").prop("disabled", true);
    }
}
/* Short Sheet
name - 100
...
bootstrapValidate(
    ...
    'regex:^[a-zA-Z ]*$:Only Alpha (a-Z) and Space (\' \') characters are allowed.|max:100:Your Project Name must not be longer than 100 characters|min:5:Your Project Name must not be shorter than 5 characters|required:Please enter a Project Name.',
    function (isValid) {
