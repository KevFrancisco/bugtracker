...
def login():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("f_user_name"):
            return render_template("login.html", msg_error="Please enter a username.")

        # Ensure password was submitted
        elif not request.form.get("f_user_password"):
            return render_template("login.html", msg_error="Please enter a password.")
        ...
        if not userrow or not check_password_hash(userrow.user_hash, l_password):
            return render_template(
                "login.html",
                error="Invald username and/or password. Please check your credentials.",
            )
        ...
        return redirect(url_for("overview_main", msg_success=msg_success))
...
def projects_new():
    ...
    if request.method == "POST":
        ...
        userlist = User.query.filter(
            (User.user_role == "Project Manager") | (User.user_role == "Admin")
        ).all()
        ...
        return render_template(
            "/projects/project_new.html", msg_error=msg_error, userlist=userlist
        )
...
def tickets_search(tck_column, str_q):
    ...
    if tck_column == "type":
        ticketslist = Ticket.query.filter_by(tck_type=str_q).order_by(
            Ticket.tck_created.desc()
        )
    elif tck_column == "priority":
        ticketslist = Ticket.query.filter_by(tck_prio=str_q).order_by(
            Ticket.tck_created.desc()
        )
    elif tck_column == "status":
        ticketslist = Ticket.query.filter_by(tck_status=str_q).order_by(
            Ticket.tck_created.desc()
        )
    else:
        return apology("Internal Server Error", 500)

    return render_template("/tickets/manage_tickets.html", ticketslist=ticketslist)
...
def tickets_search_with_project(url_proj_id, tck_column, str_q):
    ...
    if tck_column == "type":
        ticketslist = Ticket.query.filter(
            (Ticket.tck_type == str_q) & (Ticket.tck_proj_id == url_proj_id)
        ).order_by(Ticket.tck_created.desc())
    elif tck_column == "priority":
        ticketslist = Ticket.query.filter(
            (Ticket.tck_prio == str_q) & (Ticket.tck_proj_id == url_proj_id)
        ).order_by(Ticket.tck_created.desc())
    elif tck_column == "status":
        ticketslist = Ticket.query.filter(
            (Ticket.tck_status == str_q) & (Ticket.tck_proj_id == url_proj_id)
        ).order_by(Ticket.tck_created.desc())
    else:
        return apology("Internal Server Error", 500)
...
def ticktets_new():
    ...
    if request.method == "POST":
        ...
        subuserlist = User.query.filter(
            (User.user_role == "Submitter") | (User.user_role == "Admin")
        ).all()
        devuserlist = User.query.filter(
            (User.user_role == "Developer") | (User.user_role == "Admin")
        ).all()
        projectslist = Project.query.all()
        ...
        if session["user_role"] not in ["Admin", "Submitter"]:
            msg_error = (
                "Only User's with the Admin or Submitter Role may log new Tickets"
            )
...
def manage_ticket(url_tck_id):
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("tickets_main", msg_success=msg_success, msg_error=msg_error)
        )
...
def manage_user(url_user_id):
    ...
    if request.method == "POST":
        ...
        userlist = User.query.with_entities(
            User.user_id, User.user_name, User.user_first_name, User.user_last_name
        ).all()
        ...
        return render_template("/users/manage_user.html", userlist=userlist, user=user)
...
def delete_user(url_user_id):
    ...
    if session["user_role"] == "Admin":
        ...
        return render_template(
            "/users/manage_users.html", msg_error=msg_error, userlist=userlist
        )
